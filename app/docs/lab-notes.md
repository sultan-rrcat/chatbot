> Museum note: redacted copy of root Readme.md (643-line lab notes). Secrets replaced with placeholders - see app/.env.example. Original strings remain only in git history.

---

# Computer Division â€” Lab Notes
...
Sample promptâ€“reference pairs used for evaluation:
...
**Windows â€” GPT-2 XL on MMLU (with local dataset path):**
```bash
lm-eval --model hf \
...
|---|---|---|---|---|
| CPU inference | âœ… All | ðŸ¢ Slow | âŒ None | Debugging, testing |
| FP16/BF16 | ~50% | ðŸš€ Fast | âœ… Same | Modern GPU, low memory |
| Quantization (8/4-bit) | ~70â€“85% | âš¡ Fast | ðŸ”¸ Slight drop | Production inference |
| `device_map=auto` | âœ… Flexible | âš ï¸ Depends | âœ… Same | Shared CPU+GPU deployment |
| DeepSpeed/Zero-3 | ðŸ”¥ Huge | âœ… Fast | âœ… Same | Training, very large models |
...
```bash
wget --header="Authorization: Bearer <redacted-hf-token-2>" \
...
9. No external internet-level dependencies (CDN/APIs) â€” works fine on internal networks
...
1. Gradual increase in response time due to chat history (backend deque limited to last 5 turns â€” may lose context in long conversations)
...
- **SQL querying** â€” for real-time and historical data from MS SQL Server
- **RAG** â€” for contextually rich responses using fault-related tables and documentation
...
```powershell
$env:HTTP_PROXY  = "http://<user>:<pass>@<proxy-host>:<proxy-port>"
$env:HTTPS_PROXY = "http://<user>:<pass>@<proxy-host>:<proxy-port>"
```
...
Avoid Conda shells â€” use Command Prompt or PowerShell without Conda initialised.

### Permanent Fixes

**A. Remove Conda from system PATH (optional but clean)**

Remove from `Environment Variables â†’ System PATH`:
```
