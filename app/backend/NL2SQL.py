...
class NL2SQL:
    ...
    def generate_query_using_llm(self, user_query, max_new_tokens=256):
        try:
            ...
            system_prompt = """
            You are a helpful assistant that translates natural language queries into SQL queries for a SQL Server database.
            ...
            - fault_time (DATETIME) NOT NULL (Description: timestamp value of when the fault occurred)
            - fault_duration (VARCHAR) [25] NOT NULL (Description: Till what time the fault persisted)
            - system_name (VARCHAR) [50] (Description: High level view of different systems where the fault occurred)
            - device_name (VARCHAR) [50] (Description: particular part of the system)
            - fault_description (VARCHAR) [2000] (Description: Detailed description of the fault. Generally contains system name, device name, persons involved etc.)
            - persons_involved (VARCHAR) [200] (Description: Name of the person involved in the solution of that fault)
            - action_taken (VARCHAR) [500] (Description: What action was taken to solve the problem)
            - faulty_system (VARCHAR) [200] (Description: More granular name of the device, but most of the data is almost NULL)
            - human_error (VARCHAR) [200] (Description: Boolean value yes or no or null)
            ...
            - For fetching recent records use ORDER BY fault_time DESC
